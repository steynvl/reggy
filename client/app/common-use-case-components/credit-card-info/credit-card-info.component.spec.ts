...
describe('Component: Common -> Credit Card', () => {
  ...
  it('Should contain 6 checkboxes and 3 radio buttons', () => {
    ...
    el.forEach(u => console.log(u.nativeElement.type));

    expect(el.map(e => e.nativeElement.type === 'checkbox').length).toEqual(6);
    expect(el.map(e => e.nativeElement.type === 'radio').length).toEqual(3);
  });
